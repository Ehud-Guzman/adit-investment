...
export default function AboutValues() {
  return (
    <section className="relative py-20 px-4 sm:px-6 lg:px-20 bg-white text-gray-900 border-t border-gray-100">
      {/* Background Pattern */}
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
          {values.map((value, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`relative bg-white border-t-4 ${value.border} rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden p-6 sm:p-7 md:p-8`}
            >
              {/* Glow Effect */}
              <div
                className={`absolute -top-10 -right-10 w-32 h-32 rounded-full ${value.bgBlur} blur-2xl`}
              />

              {/* Badge */}
              <div
                className={`w-12 h-12 mb-5 flex items-center justify-center text-lg font-bold rounded-full ${value.bgBlur} ${value.text}`}
              >
                {i + 1}
              </div>
