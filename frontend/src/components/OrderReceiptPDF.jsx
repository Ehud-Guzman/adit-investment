...
const OrderReceiptPDF = ({ order }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `Adit_Receipt_${
          fullOrder._id?.slice(-8).toUpperCase() || "UNKNOWN"
        }.pdf`
      );
    } catch (err) {
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 15, marginBottom: 20 }}>
          ...
          >
            <h3 style={{ margin: 0, color: colors.primary, fontSize: 14 }}>
              Customer Info
            </h3>
            <p>Name: {userName}</p>
            <p>Email: {userEmail}</p>
        ...
        >
          <thead style={{ background: colors.primary, color: "#fff" }}>
            <tr>
              ...
              <th style={{ padding: "8px 5px", textAlign: "right" }}>
                Unit Price
              </th>
          ...
          <tbody>
            {orderItems.length > 0 ? (
              orderItems.map((item, idx) => (
                <tr
                  key={idx}
                  style={{ background: idx % 2 === 0 ? "#fff" : colors.accent }}
                >
              ...
              <tr>
                <td
                  colSpan={4}
                  style={{ padding: "8px 5px", textAlign: "center" }}
                >
                  No items available
                </td>
              </tr>
